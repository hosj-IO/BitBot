...
var dbHelper = require('./db/SQLiteHelper');
...
storage.prototype.push = function (csArray, callback) {
  ...
  if (databaseType == 'SQLite') {
    ...
    dbhelper.push(csArray, callback);
  } else {
...
storage.prototype.getLastNCandles = function (N, callback) {
  ...
  if (databaseType == 'SQLite') {
    var dbhelper = new dbHelper();
    dbhelper.getLastNCandles(N, callback);
  } else {
    var csDatastore = mongo(this.mongoConnectionString);
    var csCollection = csDatastore.collection(this.exchangePair);
...
storage.prototype.getAllCandles = function (callback) {
  ...
  if (databaseType == 'SQLite') {
    var dbhelper = new dbHelper();
    dbhelper.getAllCandles(callback);
  } else {
    var csDatastore = mongo(this.mongoConnectionString);
    var csCollection = csDatastore.collection(this.exchangePair);
...
storage.prototype.calculateAggregatedCandleStick = function (period, relevantSticks) {
  ...
  currentCandleStick.high = _.max(relevantSticks, function (relevantStick) {
    return relevantStick.high;
  }).high;
  currentCandleStick.low = _.min(relevantSticks, function (relevantStick) {
    return relevantStick.low;
  }).low;
  currentCandleStick.close = relevantSticks[relevantSticks.length - 1].close;
  currentCandleStick.volume = tools.round(_.reduce(relevantSticks, function (memo, entry) {
    return memo + entry.volume;
  }, 0), 8);
  if (currentCandleStick.volume === 0) {
    currentCandleStick.vwap = currentCandleStick.close;
  } else {
    currentCandleStick.vwap = tools.round(_.reduce(relevantSticks, function (memo, entry) {
        return memo + (entry.vwap * entry.volume);
      }, 0) / currentCandleStick.volume, 8);
  }
...
storage.prototype.aggregateCandleSticks = function (candleStickSize, candleSticks) {
  ...
  var filterOnVolume = function (candleStick) {
...
storage.prototype.setInitialBalance = function (initialBalance, callback) {
  ...
  csCollection.update({exchangePair: this.exchangePair}, {
    exchangePair: this.exchangePair,
